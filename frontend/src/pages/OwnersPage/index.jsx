...
export default function OwnersPage() {
    ...
    async function handleSubmit(event) {
        ...
        // --- ALTERAÇÃO 1: Validação para impedir email sem @ ---
        // Aqui verificamos se o texto digitado no campo de email não possui o caractere "@".
        // Se não possuir, bloqueamos o salvamento (return) e avisamos o usuário.
        if (!form.email.includes('@')) {
            setMessage('Por favor, insira um e-mail válido contendo "@".');
            return;
        }
    ...
    const filteredOwners = owners.filter((owner) => {
        ...
        return (
            ...
            // --- ALTERAÇÃO 3: Buscar também pelo endereço ---
            // Adicionada a validação para que o termo pesquisado verifique o campo address.
    ...
    return (
        <div>
            ...
            {/* --- ALTERAÇÃO 2 : Botão para limpar a mensagem exibida --- */}
            {message && (
                <div className="message-container">
                    <p className="message-text">{message}</p>
                    <button type="button" className="message-btn" onClick={() => setMessage('')}>
                        Limpar aviso
                    </button>
                </div>
            )}
            ...
            {/* --- ALTERAÇÃO 3 (Texto): Atualizei o placeholder para mostrar que busca endereço --- */}
            <input
            ...
            {/* --- ALTERAÇÃO 4 e 5: Exibir a quantidade total de donos acima da tabela --- */}
            <div style={{ marginTop: '10px', marginBottom: '20px' }}>
                <p style={{ fontWeight: 'bold', fontStyle: 'italic', margin: 0 }}>
                    Total de donos cadastrados: {owners.length}
                </p>
            </div>
