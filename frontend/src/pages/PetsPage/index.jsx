...
export default function PetsPage() {
    ...
    async function handleSubmit(event) {
        ...
        // 1. REGRA: Impedir cadastro com peso menor ou igual a zero
        if (Number(form.weight) <= 0) {
            setMessage('O peso do pet deve ser maior que zero.');
            return;
        }
    ...
    const filteredPets = pets.filter((pet) => {
        ...
        return (
            ...
            // 2. REGRA: Localizar pets também pelas observações
    ...
    return (
        <div>
            ...
            {/* 4. REGRA: Componente de estilização da mensagem aplicado */}
            {message && (
            ...
            >
                <option value="">
                    {owners.length === 0 ? 'Nenhum dono cadastrado' : 'Selecione'}
                </option>
                {owners.map((owner) => (
                    <option key={owner.id} value={owner.id}>
                        {owner.name}
                    ...
                    <tbody>
                        {filteredPets.map((pet) => (
                            <tr key={pet.id}>
                                ...
                                {/* 3. REGRA: Mensagem de dono não cadastrado na tabela */}
                                <td>{pet.owner?.name || 'Sem dono cadastrado'}</td>
                                <td>
            ...
            {detailPet && (
                <div>
                    ...
                    {/* 3. REGRA: Mensagem de dono não cadastrado nos detalhes */}
                    <p><strong>Dono:</strong> {detailPet.owner?.name || 'Sem dono cadastrado'}</p>
